...
- 定义好的模板语法(v-if,v-for,v-once,v-html),存在语法限制，不够自由
- 也正因为有语法限制，有一定优化方案介入(静态标记)
...
##### 2.react 纯 vdom,没有响应式

- 单向数据流
- 不可变数据
- fiber 时间切片，任务可以切开，利用浏览器空闲时间分段执行，diff 过程可以分段执行
