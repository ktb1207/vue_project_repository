    ...
    <div class="tree-wrp">
        <div class="define-table">
            <div class="define-body">
                <div class="define-header-row" :class="{'define-header-bottom-border':!existData}" :style="{'padding-right':scrollWidthPx}">
    ...
    export default {
        ...
        watch:{
            ...
            spreadNum(newVal,oldVal){//监听切换展开层级
                if(newVal==999){//当前展开状态不变
                    this.saveSpreadNum=oldVal;
                    return
                }
                if(newVal==this.saveSpreadNum){//两次展开数据一样，强制刷新节点
                    if(newVal>1){
            ...
            formaterTreeData(newVal,oldVal){
                if(newVal.length==0){
                    this.$emit('clickRow',{},'undefind','undefined','notree');
                }
            }
        },
        methods:{
            ...
            nodeMove(deriction,pid,id){
                if(pid!=null){
                    let parentItem = findTreeList(this.formaterTreeData,pid);//父节点
                    let parentChildren=parentItem.children,
                        parentChildLeng=parentItem.children.length;
                    ...
                    if(deriction=='up'&&targetItemIndex==0){
                        console.log('到顶了...')
                    }else if(deriction=='down'&&targetItemIndex==parentChildLeng-1){
                        console.log('到底了...');
                    }else{
                   ...
                   let rootNodeLeng=this.formaterTreeData.length;
                   ...
                   if(deriction=='up'&&targetItemIndex==0){
                       console.log('到顶了...')
                   }else if(deriction=='down'&&targetItemIndex==rootNodeLeng-1){
                       console.log('到底了...');
                   }else{
                       this.executeNodeMove(deriction,pid,id);
                   }
            ...
            executeNodeMove(deriction,pid,id){
                let someParentItemArr=[],someParentIndexArr=[];
                this.saveTreeData.forEach((item,index)=>{
                    if(item.pid==pid){
                ...
                someParentItemArr.forEach((item,index)=>{
                    if(item.id==id){
                        let targetIndex=someParentIndexArr[index];
                        let newIndex=deriction=='up'?someParentIndexArr[index-1]:someParentIndexArr[index+1];
                        this.$set(this.saveTreeData,newIndex,item);
                        if(deriction=='up'){
                            this.$set(this.saveTreeData,targetIndex,someParentItemArr[index-1])
                        }else{
                            this.$set(this.saveTreeData,targetIndex,someParentItemArr[index+1])
                        } 
                    }  
            ...
            computedScrollWidth(){
                if(this.$refs[this.uniqueRef]){
                    let scrollbarWidth = this.$refs[this.uniqueRef].offsetWidth - this.$refs[this.uniqueRef].clientWidth;
            ...
            deepUpConcat(obj,status){
                if(obj[this.parentIdName]){
                    let parentItem = findTreeList(this.formaterTreeData,obj.pid);//父节点
                    if(status){
                        let parentChildrenStatusArr = [];
                        let parentChildrenBanArr=[];
                        ...
                        if(parentChildrenStatusArr.includes(false)||parentChildrenBanArr.includes(true)){
                        ...
                        // 子节点有勾选项或者有半勾选项
                        if(parentChildrenStatusArr.includes(true)||parentChildrenBanArr.includes(true)){
                            this.$set(parentItem,'banCheck',true)
                        }else{
                            this.$set(parentItem,'banCheck',false)
                        }
