...
<template>
  ...
  >
    <div
      ...
      :class="{
        'active-row': contentObj.selected,
        'tree-click-node': rowClick
      }"
      ...
      >
        ...
        >
          ...
          <span
            ...
            :class="{
              'tree-name-select': contentObj.selected,
              'tree-click-node': rowClick
            }"
            :data-type="col.key"
            :data-value="contentObj[col.key] || ''"
          ...
          >
            <input
              ...
              @blur="
                editTableCell($event, contentObj[col.key], col.key, contentObj)
              "
            />
          </span>
          <!-- 不可编辑 -->
          ...
          >
            <span
              v-if="col.formater"
              v-html="col.formater(contentObj)"
            ></span>
...
export default {
  ...
  computed: {
    ...
    nodePadding() {
      ...
      let num = this.domIndex * (10 + this.domIndex + this.domIndex);
      return num + 'px';
    },
    ctrlNodeShow() {
      //节点显示or隐藏
